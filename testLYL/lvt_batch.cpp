...
int main(int argc, char** argv) {
    ...
    LVT<MultiIOBase>::initialize_batch(func_name, lvt_raw, num_party, party, io.get(), &pool, elgl.get(), alpha_fr, num, m_bits);
    lvt.reset(lvt_raw);
    ...
    std::string input_base = "../Input/Input-P";
    ...
    if (party == 1) {
        std::string output_base = "../Output/Output";
